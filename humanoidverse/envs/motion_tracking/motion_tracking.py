...
class LeggedRobotMotionTracking(LeggedRobotBase):
    def __init__(self, config, device):
        ...
        if self.config.use_teleop_control:
            self.teleop_marker_coords = torch.zeros(
                self.num_envs,
                3,
                3,
                dtype=torch.float,
            ...
            self.teleop_sub = self.node.create_subscription(
                Float64MultiArray, "vision_pro_data", self.teleop_callback, 1
            )

        if (
            self.config.termination.terminate_when_motion_far
            and self.config.termination_curriculum.terminate_when_motion_far_curriculum
        ):
        ...
        if "save_motion" in self.config:
            ...
            if self.save_motion:
                ...
                if hasattr(self.config, "dump_motion_name"):
                    self.save_motion_dir = (
                        Path(self.config.ckpt_dir)
                        / "motions"
                        / (
                            str(self.config.eval_timestamp)
                            + "_"
                            + self.config.dump_motion_name
                        )
                    )
                else:
                    self.save_motion_dir = (
                        Path(self.config.ckpt_dir)
                        / "motions"
                        / f"{self.config.save_note}_{self.config.eval_timestamp}"
                    )
    ...
    def _update_terminate_when_motion_far_curriculum(self):
        ...
        if (
            ...
            < self.config.termination_curriculum.terminate_when_motion_far_curriculum_level_down_threshold
        ):
            self.terminate_when_motion_far_threshold *= (
                1
                + self.config.termination_curriculum.terminate_when_motion_far_curriculum_degree
            )
        elif (
            self.average_episode_length
            > self.config.termination_curriculum.terminate_when_motion_far_curriculum_level_up_threshold
        ):
            self.terminate_when_motion_far_threshold *= (
                1
                - self.config.termination_curriculum.terminate_when_motion_far_curriculum_degree
    ...
    def _check_termination(self):
        ...
        if self.config.termination.terminate_when_motion_far:
            reset_buf_motion_far = torch.any(
                torch.norm(self.dif_global_body_pos, dim=-1)
                > self.terminate_when_motion_far_threshold,
                dim=-1,
            )
    ...
    def _pre_compute_observations_callback(self):
        ...
        self.ref_body_pos_extend[:] = (
            ref_body_pos_extend  # for visualization and analysis
        )
        ref_body_vel_extend = motion_res["body_vel_t"]  # [num_envs, num_markers, 3]
        self.ref_body_rot_extend = ref_body_rot_extend = motion_res[
            "rg_rot_t"
        ...
        self.dif_global_body_ang_vel = (
            ref_body_ang_vel_extend - self._rigid_body_ang_vel_extend
        )
        ...
        heading_inv_rot_expand = (
            ...
            .expand(-1, num_rigid_bodies + self.num_extend_bodies, -1)
            .reshape(-1, 4)
        )
        ...
        heading_rot_expand = (
            heading_rot.unsqueeze(1).expand(-1, num_rigid_bodies, -1).reshape(-1, 4)
        )

        dif_global_body_pos_for_obs_compute = ref_body_pos_extend.view(
            env_batch_size, -1, 3
        ) - self._rigid_body_pos_extend.view(env_batch_size, -1, 3)
        ...
        global_ref_rigid_body_pos = ref_body_pos_extend.view(
            env_batch_size, -1, 3
        ) - self.simulator.robot_root_states[:, :3].view(
            env_batch_size, 1, 3
        ...
        if not self.config.use_teleop_control:
            ref_vr_3point_pos = ref_body_pos_extend.view(env_batch_size, -1, 3)[
                :, self.motion_tracking_id, :
            ]
        else:
            ref_vr_3point_pos = self.teleop_marker_coords
        vr_2root_pos = ref_vr_3point_pos - self.simulator.robot_root_states[
            :, 0:3
        ].view(env_batch_size, 1, 3)
        heading_inv_rot_vr = heading_inv_rot.repeat(3, 1)
        self._obs_vr_3point_pos = my_quat_rotate(
            heading_inv_rot_vr.view(-1, 4), vr_2root_pos.view(-1, 3)
        ).view(env_batch_size, -1)
        #################### Deepmimic phase ######################
        ...
        if not (
            torch.all(self._ref_motion_phase >= 0)
            and torch.all(self._ref_motion_phase <= 1.05)
        ...
        for env_id in range(self.num_envs):
            if not self.config.use_teleop_control:
                ...
                for pos_id, pos_joint in enumerate(self.teleop_marker_coords[env_id]):
                    self.simulator.draw_sphere(
                        pos_joint, 0.04, (0.851, 0.144, 0.07), env_id, pos_id
                    )
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.custom_origins:  # trimesh
            ...
            self.simulator.robot_root_states[env_ids, 7:10] = motion_res["root_vel"][
                env_ids
            ]
            self.simulator.robot_root_states[env_ids, 10:13] = motion_res[
                "root_ang_vel"
            ][env_ids]
        ...
        if self.save_motion:
            ...
            if (len(self.motions_for_saving["dof"])) > self.config.save_total_steps:
                for k, v in self.motions_for_saving.items():
                    self.motions_for_saving[k] = (
                        torch.stack(v[3:]).transpose(0, 1).numpy()
                    )

                self.motions_for_saving["motion_times"] = (
                    torch.stack(self.motion_times_buf[3:]).transpose(0, 1).numpy()
                )
            ...
            root_rot_vec = torch.from_numpy(
                sRot.from_quat(root_rot.numpy()).as_rotvec()
            ).float()
            ...
            pose_aa = torch.cat(
                [
                    root_rot_vec[:, None, :],
                    self._motion_lib.mesh_parsers.dof_axis * dof[:, :, None],
                    torch.zeros((self.num_envs, self.num_augment_joint, 3)),
                ],
                axis=1,
            )
